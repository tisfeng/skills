...
description: >
  ...
  合并最新 base 分支。理解 PR 和关联 issue 的目标，复用 review 核心核对功能、实现方式与代码正确性，并维护有证据的线程状态；
...
- 首次快照使用一个 helper 先冻结 PR 元数据和 head，再并行收集直接关联的 issue 正文、
  ...
  `context` section 保存问题来源，不代表已经理解需求或验证功能；缺失时按下文补齐，不能
...
- 首次语义审查后冻结 remote head、base SHA、merge-base、changed paths、raw diff 和四类远程
...
- 最终刷新仍重新读取 PR、选定 issue 证据、完整 threads/replies 和 checks，但以 fingerprint 压缩未变化输出：
  ...
  默认只返回变化的 section：关联 issue 正文或已选讨论变化只返回 `context`，线程变化只返回
  线程增量，不会重发未变化的 `pr`、diff 相关证据或 checks；head 变化时返回全部当前证据。
  提供已验证的前次快照时，`context_delta` 与 `threads_delta` 只携带变化来源的完整正文、
  变化和移除清单，以及当前全量索引，未变化来源只保留索引。base 参数成对提供，不能把
  `base_comparison: not_provided` 当作 base 未变；旧 helper 路径需手动比较 base 名称和 SHA，
  按下文重新确定审查范围。
...
review threads，不要紧接着重复查询相同内容。复用 `context` section 的 issue 正文；存在需求
...
- 即使 head 未变，标题、描述、issue 关联集合、正文或已采用的讨论变化，也须重新核对受影响
  ...
  宣称功能结论仍成立。这类变化只返回 `context`，提供已验证前次快照时为 `context_delta`；
  据增量更新目标与验收判断，不重读未变化的 PR 证据。来源读取失败、旧快照缺失或讨论未覆盖时
  保留明确限制。
...
目标明确后，还要判断所选实现方式是否足够好：结合 PR 目标、关键验收条件和仓库既有模式，
比较当前做法与可行替代的范围、复杂度、代价和引入的风险，并在同一份报告中给出判断。当前
方案存在具体风险或缺陷时，按 P 级 finding 说明并给出替代方案；没有缺陷但确实存在更优的
低风险替代时，放入待确认决策由用户决定是否采纳；找不到更好方案时明确说明当前方式已经足够，
不为结论制造建议。只评价与本次目标相关的实现路径，不扩大为整体重构，也不因个人风格偏好
要求修改。
...
1. **审查结论**：先分别说明功能目标的满足程度、实现方式评估和代码审查建议，再给主要原因
   及下一步。功能目标区分已满足、部分满足、明确缺口或证据不足，并说明是静态证据还是实际
   运行验证。实现方式说明当前路径是否足以达成目标，或指出更优替代及其取舍；没有更优替代时
   明确说明当前方式已经足够，不留空。CI 失败/pending、未完成的
...
3. **待确认决策**：只放影响正确性/范围且需要用户选择的事项，给出推荐与主要取舍；
   更优实现方式不构成当前缺陷、只需用户决定是否采纳时也放这里。关联已有问题时引用 ID，
   不重写其证据和修复方案。没有决策就省略。
...
- 实现方式评估：当前路径、比较依据和取舍；给出替代方案时说明影响范围与代价，没有替代时
  写出“当前方式已足够”的可核对结论，不只留空。
