...
Agent。委派时优先使用独立上下文，只传当前授权、限制、路径、冻结快照、验证结果及实际
Skill 路径；同一轮已读且未变化的规则不重复读取。

### 已有提交的一次委派

主 Agent 已确认 `operation=integration`，且源已有待集成提交、源及所选目标 worktree 干净、
无需新提交、用户没有确认、仅预览、仅草稿或暂缓限制时，可委派 `phase=integrate`。
这是内部阶段名，表示“检查通过后继续集成”，不需要用户输入额外命令。

执行者在同一次委派内完成下面的只读预检、冻结范围、写入前复验和集成，不在检查通过后
返回主 Agent 等待第二次派发。若尚无目标 worktree，允许按本 Skill 创建临时目标 worktree。
脚本成功只代表已收集证据；授权、路径和提交限制仍由执行者逐项核对。

- 发现需要新提交时，返回 `needs-prepare` 和只读证据，改走下方提交预览流程；不自行暂存或提交。
- 发现脏目标、范围不符、状态漂移或冲突时，停止写入并保留现场。此路径遇到任何 rebase/merge
  冲突都返回主 Agent；不在同次连续执行中自动解决冲突或改用其他提交范围。
- 条件不满足时，不为尝试连续集成而先创建源分支或临时 worktree。
- 需要确认或只读预览时使用 `prepare`；已有提交无需起草新 message，但仍遵守用户的等待限制。
...
2. 若需要新提交，主 Agent 必须在主对话中原样显示 `提交信息预览` 和完整草稿。默认模式
   ...
   `preexisting-source-commit`，无需提交信息预览；适用上面条件时可直接选择一次委派。
...
委派。分阶段交付时，只有完全位于允许路径且不需产品语义判断的机械冲突可以处理，其他冲突必须保留
现场并返回主 Agent。

### 集中检查与阶段复验

`phase=integrate` 使用随本 Skill 分发的
[inspect-integration.py](scripts/inspect-integration.py) 收集只读证据。脚本只检查已有提交的
连续集成；需要新提交或源、目标同分支时，沿用 `git-commit` 的候选冻结、暂存策略和
`prepare/apply` 复验，不把下面的脚本退出码作为该提交流程的门禁。

```bash
python3 "<skill-dir>/scripts/inspect-integration.py" snapshot \
  --source "<source-worktree>" --target "<target-branch>"
python3 "<skill-dir>/scripts/inspect-integration.py" verify --snapshot -
```

`<skill-dir>` 是实际加载的本 Skill 目录。`verify` 从 stdin 读取先前完整 JSON，也可指定
仓库外的 JSON 文件。脚本自身只向 stdout 输出，不创建快照文件或修改 Git 状态。
省略 `--target` 时按下方规则查询实时远程默认分支；同一轮 `verify` 复用已解析目标，不重复
联网。恢复暂停的任务时重新解析目标。detached 时可用 `--source-branch` 检查按
`git-commit` 命名规则推导的候选，返回创建或复用建议，不实际创建分支。

JSON 包含完整 HEAD、symbolic ref、Git 元数据路径、worktree 状态与占用、进行中的操作、
merge-base、全部待集成提交及每个提交触及路径的并集（包括后来被撤销的变更）。退出码
`0` 表示证据无阻塞，`2` 表示状态阻塞或快照漂移，`1` 表示读取或参数错误；后两者均不能继续
连续集成写入。脚本不能判断产品语义、用户授权或沙箱写入权限，也不能提前保证 rebase 无冲突。

- 同一阶段复用上述证据，不重复逐条运行下方已有的等价检查命令；必要时才补充缺失信息。
- 创建分支、rebase、创建目标 worktree 和 merge 仍串行执行，每次写入前即时复验相关状态。
  `verify` 比较整个阶段快照，不是跨命令的锁。
- 每次写入后，先核对实际变化是否恰好符合已授权操作，再建立下一阶段基线。挂接只允许
  源分支身份及对应占用变化，源 HEAD 和内容必须不变；创建临时目标只允许新增该目标占用，
  目标 HEAD 必须不变。不可用重新 snapshot 无条件接受其他漂移。
- rebase 后确认源干净、目标仍为冻结的 HEAD，并用原始与新提交范围核对改写结果。merge 前
  的基线使用已核验的 rebase 后源 HEAD，同时保留原目标 HEAD、目标分支身份和干净状态。
  目标 ref 前进、checkout 切换或新增占用都要求停止，不隐式再次 rebase。

### 已知权限边界

结合当前环境权限与快照中的 `git_dir`、`common_dir`、源/目标 worktree 路径判断写入位置。
已知位置受限时，直接为相应的已授权 Git 命令申请最小必要提权；同一轮已确认的权限边界可
复用，避免每次先运行必然失败的普通权限命令。权限审批仍逐操作适用，不从文件系统可写或
先前命令获批推断新增业务授权。审批拒绝时保留现场并报告受阻命令与原因，不绕过审批。
...
- `git-commit` 是提交结果、统计表和单提交实际 message 的唯一模板来源；本 Skill 只补充
  ...
  回执，不得压缩执行者的结果。
- 执行者一次收集实际 Git message、完整哈希、最终状态与统计脚本结果；主 Agent 批量独立
  核验后复用这些数据呈现上述完整格式，不重新起草或翻译实际提交信息。多提交统计使用冻结的
  合并前目标 OID 与 rebase 后源 OID，避免目标已前进后读出空范围。
