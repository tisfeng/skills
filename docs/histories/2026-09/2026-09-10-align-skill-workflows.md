...
- 日期：2026-09-10
- 状态：completed
- 关联计划：[执行计划](../../exec-plans/completed/2026-09/2026-09-10-align-skill-workflows.md)
...
用户担心 `f3f585f` 中的删除影响功能，逐段对照及独立审查确认少量文字要求未完整承接；
用户随后授权补回。此次以 `f3f585fb2f3b2e8a5ae2b9dc075954dea3e8c1b0` 为基线，初始工作树
和索引干净，仅恢复三个 Skill 文档中的要求，并更新本记录。

- `submit-pr` 的默认 `neutral` 策略重新约束调用 Agent 不主动生成自动关闭 Issue 语法，保留
  目标模板或用户显式附加正文的例外；不把该语义要求误当作 helper 已有的强制校验。
- 正文标准集中在所有模式必读的工作流契约，恢复改动原因和实际验证结果；纯 plan 明确展示
  完整预览后停止。默认/draft 仍先完成获准的 staged 提交和 base fetch，再起草最终预览。
- `review-pr` 恢复需求目标、持久化、本地化、平台兼容及无关改动等简短审查重点。
- 复用前一轮独立规划；本次独立只读复核无 finding。PR review 27 项、PR 提交 23 项测试、
  6 个 Skill 结构校验、两个修改 Skill 的 quick_validate 与 diff 检查通过。
- 对比确认脚本、测试、技能描述、线程处理、最终刷新及完整报告格式未改动。此次验证覆盖文字
  契约和现有脚本行为，未运行真实 PR 或独立模型行为场景；测试通过不代表指令天然等价。
