...
3. 当前主 Agent 直接、串行执行 Git 操作：提交使用 `git-commit`，集成使用
   ...
   暂存策略、候选内容复验和唯一暂存步骤以 `git-commit` 为准。运行时支持程序化工具调用时，
   合并正常成功路径的模型往返：并行无依赖只读检查，并在一次程序化调用中依次等待独立命令；
   不合并逐命令权限、退出状态、写前复验、失败停止或恢复边界。
...
6. 按 Skill 核验提交和集成结果，并呈现完整回执。实际提交信息直接读取 Git，不重新起草或
   ...
   首次完整 raw patch 仍用于语义审核；后续未变化事实使用内容证据复验并紧凑返回，避免重复
   输出全量 refs、worktrees、status、log 或 diff。运行中会话和缺少退出码都不能视为成功。
