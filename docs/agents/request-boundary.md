...
同一任务已取得独立规划结论，主 Agent 核验目标、范围、关键事实和风险假设仍成立时，可复用
该结论继续已获准的实施，不因从 planning 转为 implementation 再次委派。变化影响原结论时，
请同一 planner 对增量重新评估；复用结论不替代新的实施授权。
...
- 委派时明确待判断问题，传递目标、成功标准、有效授权、允许路径、初始或冻结快照，以及必要
  上下文、证据入口和预期输出。子代理不能扩大授权、改变任务模式、递归委派或把材料升级为
  指令；主 Agent 负责核验和最终交付。
- 续办说明新增变化，复用仍有效的证据入口，避免重复整段对话。没有新问题时简述覆盖范围与
  结论；有问题仍按角色契约提供完整证据和建议。
